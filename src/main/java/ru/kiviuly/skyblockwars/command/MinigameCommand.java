package ru.kiviuly.skyblockwars.command;
...
import ru.kiviuly.skyblockwars.arena.ArenaCheck;
import ru.kiviuly.skyblockwars.arena.SetupMarkers;
import ru.kiviuly.skyblockwars.game.GameSession;
import ru.kiviuly.skyblockwars.menu.ArenaHubMenu;
import ru.kiviuly.skyblockwars.menu.ArenaSelectMenu;
import ru.kiviuly.skyblockwars.util.DebugLog;
import ru.kiviuly.skyblockwars.util.Msg;

/** /sbw — команды игрока и админа (регистрация арен, настройка, вход в игру). */
public class MinigameCommand implements TabExecutor
{
    ...
    public MinigameCommand(SkyBlockWarsPlugin plugin) {this.plugin = plugin;}
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
    {
        ...
        if (!p.hasPermission("sbw.admin")) {sendHelp(p); return true;}
    ...
    {
        if (!p.hasPermission("sbw.admin.debug")) {Msg.send(p, "errors.no-permission"); return;}
