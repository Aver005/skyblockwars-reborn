package ru.kiviuly.mcmgp.command;
...
import ru.kiviuly.mcmgp.arena.Arena;
import ru.kiviuly.mcmgp.arena.ArenaCheck;
import ru.kiviuly.mcmgp.arena.SetupMarkers;
import ru.kiviuly.mcmgp.game.GameSession;
import ru.kiviuly.mcmgp.menu.ArenaHubMenu;
import ru.kiviuly.mcmgp.menu.ArenaSelectMenu;
...
public class MinigameCommand implements TabExecutor
{
    ...
    private final McmgpPlugin plugin;

    public MinigameCommand(McmgpPlugin plugin) {this.plugin = plugin;}
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
    {
        ...
        if (!p.hasPermission("mcmgp.admin")) {sendHelp(p); return true;}
    ...
    {
        if (!p.hasPermission("mcmgp.admin.debug")) {Msg.send(p, "errors.no-permission"); return;}
    ...
    private void sendHelp(Player p)
    {
        ...
        if (p.hasPermission("mcmgp.admin"))
        {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args)
    {
        ...
        if (args.length == 1)
        {
            ...
            if (p.hasPermission("mcmgp.admin")) {subs.addAll(ADMIN_SUBS);}
